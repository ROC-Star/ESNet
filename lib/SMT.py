...
def smt_t(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model_dict = torch.load('/tmp/pycharm_project_835/pre/smt_tiny.pth', weights_only=False)['model']
        model_dict = {k: v for k, v in model_dict.items() if k in model.state_dict()}
        model.load_state_dict(model_dict)
        # model.load_state_dict(torch.load('/tmp/pycharm_project_835/pre/smt_tiny.pth', weights_only=False)['model'])
